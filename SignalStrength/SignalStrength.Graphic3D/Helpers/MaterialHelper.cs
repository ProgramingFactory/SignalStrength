...
namespace SignalStrength.Graphic3D.Helpers
{
    ...
    public static class MaterialHelper
    {
        ...
        {
            return CreateMaterialWithOpacity(brush, opacity, 1D);
        }


        public static Material CreateMaterialWithOpacity(Brush brushOrgin, double opacity, double specularBrightness, double specularPower = 100, byte ambient = 255, bool freeze = true)
        {
            var brush = brushOrgin.CloneCurrentValue();
            var mg = new MaterialGroup();
            brush.Opacity = opacity;
            mg.Children.Add(new DiffuseMaterial() { Brush = brush, AmbientColor = Color.FromArgb((byte)(opacity * 255), ambient, ambient, ambient) });
            if (specularPower > 0)
            {
                var b = (byte)(255 * specularBrightness);
                mg.Children.Add(new SpecularMaterial(new SolidColorBrush(Color.FromRgb(b, b, b)), specularPower));
